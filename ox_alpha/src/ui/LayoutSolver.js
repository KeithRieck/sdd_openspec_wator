 ...
 * @param {number} gridAspect Grid width / height ratio.
 ...
 */
export function solveLayout(viewportWidth, viewportHeight, gridAspect) {
  ...
  if (narrow) {
    ...
    return { stats, world, controls, chart, worldScale: fitWorld(world, gridAspect), narrow: true };
  }
  ...
  return { stats, world, controls, chart, worldScale: fitWorld(world, gridAspect), narrow: false };
}

/**
 * Computes the cell size and centering offsets that fit the grid into
 * a world rectangle while preserving aspect ratio (AC 8, 52).
 *
 ...
 */
function fitWorld(world, gridAspect) {
  const cell = Math.max(1, Math.floor(Math.min(world.w / gridAspect, world.h)));
  const gridW = Math.floor(cell * gridAspect);
  const gridH = cell;
